...
const applyFilter = (contests, solved, input) => {
  ...
  const oldestTimestamp = solved.reduce(
    (value, item) => (item.timestamp ? Math.min(item.timestamp, value) : value),
    new Date().getTime(),
  )
  ...
  for (const contest of contests) {
    if (
      contest.timestamp &&
      contest.timestamp > oldestTimestamp &&
      contest.timestamp <= currentTimestamp
    ) {
  ...
  return filteredContests
    .filter(
      (contest) =>
        !contestType || contestType.prelim1 || !contest.name.match(/一次予選/),
    )
    .filter(
      (contest) =>
        !contestType ||
        contestType.prelim2 ||
        !contest.name.match(/二次予選|[^次]予選/),
    )
    .filter(
      (contest) =>
        !contestType || contestType.final || !contest.name.match(/本選/),
    )
    .filter(
      (contest) =>
